...
const containsHTMLEnties = str => /&[\d\w]+;/.test( str);
...
export const containsHTML = str => {
	//log('err', 'check if contains HTML:', str, '->', containsHTMLElements( str) || containsHTMLEnties( str))
	return containsHTMLElements( str) || containsHTMLEnties( str)
}
...
export const loadStyleSheetAsync = href => new Promise( (resolve, reject) => {
	...
	s.onerror = e => log('err','Error loading stylesheet:', href)
...
export const range = 
	(start, end) => {
		...
		if( end < start) 
			return Array.from({length: (start - end+1)}, (v, k) => k + end).reverse()
...
export function cycleDataStates( target, wrap=true){
	...
	//console.log('info', 'got states:', states)
...
export function randomNumberFromRangeBag( start, end, prefix='', initial_random_offset=false, return_bag=false, debug=false ){
	...
	if( !bag && initial_random_offset){
		bag = randomBags[ bag_id] = range( start, end)
